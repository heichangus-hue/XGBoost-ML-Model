...
class Population_vs_RMSD_Histogram():
    ...
    def draw(self):
        ...
        plt.title('Frequency Distribution of the Maximum RMSD Values for 80 Selected Proteins', fontsize=16)
        plt.xlabel("Maximum RMSD / $\mathrm{\AA}$", fontsize=14)
        plt.ylabel("Frequency (Number of Samples)", fontsize=14)
        #plt.legend()
        
        plt.tight_layout()
        plt.savefig(f"Population_vs_Max_RMSD_Histogram.png", dpi = 500)

data_file = "batch_max_rmsd_summary.csv"
