...
DATA_DIRS = [
    ...
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/2nd_batch_all",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/proteins_without_cofactors",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/proteins_without_cofactors_2",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/HEME_third_batch",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/FAD_third_batch",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/Zn_third_batch_all",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/Cu_1_third_batch_all"
]
...
SEQUENCE_FILE_PATHS = [
    ...
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/2nd_batch_all/sequences_2nd_batch.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/proteins_without_cofactors/sequences_cofactorless.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/proteins_without_cofactors_2/sequences_cofactorless_2.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/HEME_third_batch/sequences_HEME_third_batch.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/FAD_third_batch/sequences_FAD_third_batch.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/Zn_third_batch_all/sequences_Zn_third_batch.txt",
    "/mnt/iusers01/fse-ugpgt01/chem02/u28460tc/scratch/Cu_1_third_batch_all/sequences_Cu_third_batch.txt"
]
...
if __name__ == "__main__":
    ...
    print(f"Running LOOCV (Radius: {best_radius} Å)...")
    for train_idx, test_idx in loo.split(X):
        ...
        pdb_id = df_final.iloc[test_idx[0]]['PDB_Code']

        y_true.append(actual_idx)
        y_pred.append(pred_idx)

        # Logging detail for this PDB
        results_list.append({
            "PDB_Code": pdb_id,
    ...
    df_results = pd.DataFrame(results_list) # THIS LINE WAS MISSING OR MISPLACED
    ...
    for idx, row in df_results.iterrows():
        ...
        p_true = max(row[prob_cols[actual_idx]], 1e-15)  # Avoid log(0)
        loss_data.append({
            "Class": row['Actual'],
            "LogLoss": -np.log(p_true),
            "Correct": row["Correct"],
            "Max_Prob": row[prob_cols].max()
        })
...
# --- PLOT A: BOXPLOT ---
    ...
    plt.savefig("Confidence_Histogram.png", dpi=500)
    plt.close()
    ...
    plt.title(f"LOOCV Confusion Matrix for {best_radius} Å Radius \n(Accuracy: {accuracy_score(y_true, y_pred)*100:.2f}%, F1: {f1_score(y_true, y_pred, average='macro')*100:.2f}%)")
    ...
    plt.title(f"Top 10 Features (Radius: {best_radius} Å)")
    ...
    feature_corr = X.corr() # Strictly correlates X with itself
    sns.heatmap(feature_corr, annot=False, cmap='coolwarm', center=0, square=True, linewidths=.5)
    plt.title(f"Feature-Feature Correlation Matrix (N={len(selected_features)} features)")
    plt.tight_layout()
    plt.savefig("Feature_Feature_Correlation_Matrix.png", dpi=600)
    plt.close()
    ...
    # Print log loss by class
    # Merge PDB codes and Predictions back with the loss data
    df_viz['PDB_Code'] = df_results['PDB_Code'].values
    ...
    for protein_class in class_names:
        ...
        print(f"\n>>> Class: {protein_class}")
        print(f"{'PDB Code':<10} | {'Predicted':<12} | {'Correct':<8} | {'Log Loss':<8}")
        print("-" * 50)
        
        for _, row in class_subset.iterrows():
            status = "Yes" if row['Correct'] == 1 else "No" 
            print(f"{row['PDB_Code']:<10} | {row['Predicted']:<12} | {status:<8} | {row['LogLoss']:.4f}")
